  ...
  return (
    <div className="flex gap-0.5" aria-label={`${count} out of 5 stars`}>
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} className="w-3.5 h-3.5 text-[#F2A65A]" fill="currentColor" viewBox="0 0 20 20">
  ...
  return (
    <section id="testimonials" className="py-12 md:py-20 bg-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="text-[#F2A65A] text-xs font-bold uppercase tracking-widest mb-2 block">
            What Travellers Say
          </span>
          <h2 className="text-2xl sm:text-4xl font-black text-[#0B4F6C]">
          ...
          {REVIEWS.map((r, i) => (
            <motion.article
              ...
              className="bg-gray-50 rounded-2xl p-5 sm:p-6
                         ...
                         flex flex-col gap-3
                         hover:border-[#F2A65A]/40 hover:bg-amber-50/30
            ...
            >
              ...
              <div className="flex items-center gap-2.5 pt-2 border-t border-gray-200">
                {/* Initials avatar */}
                <div className="w-8 h-8 rounded-full bg-[#0B4F6C] flex items-center justify-center
              ...
              {/* Destination tag */}
              <span className="self-start bg-[#0B4F6C]/8 text-[#0B4F6C] text-xs font-semibold
